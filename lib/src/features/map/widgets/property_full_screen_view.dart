...
/// شاشة كاملة لعرض تفاصيل العقار
/// 
/// تعرض الصور في الأعلى (60%) والخريطة في الأسفل (40%)
/// مع إمكانية عرض المسافة من موقع المستخدم والعقار
class PropertyFullScreenView extends StatefulWidget {
  final PropertyModel property;
  
  const PropertyFullScreenView({
    super.key,
    required this.property,
  });
  
  @override
  State<PropertyFullScreenView> createState() => _PropertyFullScreenViewState();
}

class _PropertyFullScreenViewState extends State<PropertyFullScreenView> {
  ...
  BitmapDescriptor? _propertyMapPin;
  ...
  Future<void> _loadPropertyMapPin() async {
    ...
    if (mounted) setState(() => _propertyMapPin = icon);
  }
  ...
  Future<void> _getUserLocationAndDistance() async {
    try {
      ...
      if (_userLocation != null) {
        // ✅ حساب المسافة والوقت من موقع المستخدم إلى العقار
        final directionsService = DirectionsService();
        final routeInfo = await directionsService.getRouteInfo(
          origin: _userLocation!,
          destination: widget.property.location,
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            right: 16,
            child: _buildPropertyActionButtons(),
          ),
          ...
          Positioned(
            ...
            child: _buildMapSection(location),
          ),
        ],
      ),
    );
    ...
    return Stack(
      children: [
        ...
        GoogleMap(
          ...
          markers: {
            Marker(
              markerId: MarkerId('property_${widget.property.id}'),
              position: location,
              icon: _propertyMapPin ??
          ...
          // ✅ رسم خط بين موقع المستخدم والعقار
  ...
  Widget _buildGoogleMapsButton(LatLng location) {
    return Container(
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(12),
          onTap: () => _openInGoogleMaps(location, widget.property.address),
          child: Container(
  ...
  Widget _buildPropertyActionButtons() {
    return Column(
      mainAxisSize: MainAxisSize.min,
      ...
      children: [
        ...
        Material(
          ...
          child: InkWell(
            ...
            onTap: () {
              HapticFeedback.mediumImpact();
              MapHelpers.launchPhoneCall(widget.property.contactPhone, context);
            },
            child: Container(
              padding: const EdgeInsets.symmetric(vertical: 12, horizontal: 16),
              decoration: BoxDecoration(
